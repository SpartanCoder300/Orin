...
private extension Color {
    static let OrinGreen = Color(red: 0.204, green: 0.827, blue: 0.600)
    static let OrinAmber = Color(red: 0.961, green: 0.620, blue: 0.043)
    static let OrinRed   = Color(red: 1.000, green: 0.271, blue: 0.227)
}

private func restPhaseColor(endsAt: Date, totalDuration: TimeInterval) -> Color {
    guard totalDuration > 0 else { return .OrinGreen }
    let ratio = max(0, endsAt.timeIntervalSinceNow) / totalDuration
    if ratio > 0.5 { return .OrinGreen }
    if ratio > 0.2 { return .OrinAmber }
    return .OrinRed
}
...
struct OrinWidgetsLiveActivity: Widget {
