...
/// Nova theme surface treatment for cards.
///
/// Adds a bright border so cards read as distinct surfaces against the dark mesh.
        ...
        let withBorder = content
            .overlay {
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .strokeBorder(.white.opacity(theme == .mesh ? 0.12 : 0.08), lineWidth: 1)
            }
...
extension View {
    /// Applies the Nova theme surface treatment. Pass `exerciseIndex` on active workout
    /// cards so only the card whose set was logged receives the shimmer sweep.
